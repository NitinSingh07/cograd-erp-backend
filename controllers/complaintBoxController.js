...
exports.referComplaint = async (req, res) => {
  try {
    ...
    if (complaint.referredTo) {
      return res.status(400).json({
        message: "Complaint is already referred and cannot be referred again",
      });
    } else if (complaint.status === "RESOLVED") {
      return res.status(400).json({
        message: "Complaint is already resolved and cannot be referred",
      });
    }
    ...
    res
      .status(200)
      .json({ message: "Complaint referred to teacher", complaint });
  } catch (err) {
...
exports.complaintsReferredToTeacher = async (req, res) => {
  try {
    ...
    if (!complaints || complaints.length === 0) {
      return res
        .status(404)
        .json({ message: "No complaints found for this teacher" });
    }
...
exports.deleteComplaint = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Complaint deleted", complaint: deletedComplaint });
  } catch (err) {
